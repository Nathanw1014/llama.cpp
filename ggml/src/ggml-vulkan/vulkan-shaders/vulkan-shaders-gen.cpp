...
#if defined(GGML_VULKAN_INTEGER_DOT_GLSLC_SUPPORT)
        if (is_legacy_quant(tname) || tname == "mxfp4" || tname == "iq4_xs" || is_k_quant(tname) || tname == "iq1_s" || tname == "iq1_m") {
...
void write_output_files() {
    ...
    for (const auto& tname : type_names) {
        if (btype == "q8_1" && !is_legacy_quant(tname) && tname != "mxfp4" && tname != "iq4_xs" && !is_k_quant(tname) && tname != "iq1_s" && tname != "iq1_m") {
            continue;
        }
