...
static void ggml_vk_flash_attn(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * q, const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask, const ggml_tensor * sinks, ggml_tensor * dst) {
    ...
    // f16 K/V with the head-interleaved KV-cache layout benefits from the same one-pass
    // contiguize (dequant_f16_transpose is a pure strided copy, no dequant): the cm1
    // direct-from-global coopMatLoads run ~2x slower on those strided rows than on
    // per-head-contiguous K/V. Only engages when the rows are actually strided.
